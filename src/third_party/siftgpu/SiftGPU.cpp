...
{ 
	...
	_imgpath =  new char[MAX_PATH];
	_outpath = new char[MAX_PATH];
...
inline void SiftGPU::InitSiftGPU()
{
    ...
    if(GlobalUtil::_GoodOpenGL == 0)
    {
        ...
        LOG_ERR() << "Switch from OpenGL to CUDA\n";
    }

    if(GlobalUtil::_UseCUDA && !PyramidCU::CheckCudaDevice(GlobalUtil::_DeviceIndex))
    {
        LOG_ERR() << "Switch from CUDA to OpenGL\n"; 
        GlobalUtil::_UseCUDA = 0;
    }
#endif

	if(GlobalUtil::_verbose) LOG_OUT() <<"[SiftGPU Language]:\t" 
...
int  SiftGPU::SetSIFT(int width, int height, const void * data, unsigned int gl_format, unsigned int gl_type)
{
...
{
	if(SetSIFT(width, height, data, gl_format, gl_type))
		return RunSIFT();
	return 0;
}
int  SiftGPU::RunSIFT(int width, int height, const void * data, unsigned int gl_format, unsigned int gl_type,
					  int num, const SiftKeypoint * keys, int keys_have_orientation)
{
	if(SetSIFT(width, height, data, gl_format, gl_type))
		return RunSIFT(num, keys, keys_have_orientation, 0);
	return 0;
}
...
int SiftGPU::RunSIFT(int num, const SiftKeypoint * keys, int keys_have_orientation, int run_on_current)
{
	if(num <=0) return 0;
	_pyramid->SetKeypointList(num, (const float*) keys, run_on_current, keys_have_orientation);
	return RunSIFT();
}
...
int SiftGPU::RunSIFT()
{
	...
	timer.StopTimer();
	#endif
	if(GlobalUtil::_verbose)LOG_OUT()<<std::endl;
...
void SiftGPU::SetVerbose(int verbose)
{
	...
	if(verbose == -1)
	{
		...
		if(GlobalUtil::_verbose)
		{
			...
			if(GlobalUtil::_verbose == 0 && GlobalUtil::_UseSiftGPUEX)
				LOG_OUT() << "Console ouput disabled, press Q/V to enable\n\n";
		}else
		{
	...
	{
		//trick for disabling all output
		GlobalUtil::_verbose = 0;
		GlobalUtil::_timingS = 0;
	}else 
	{
...
void SiftGPU::ParseParam(int argc, const char **argv)
{
	...
	for(int i = 0; i< argc; i++)
	{
        ...
        case MAKEINT4(n, d, e, s):
            ...
            break;  
		case MAKEINT4(r, n, d, e):
			GlobalUtil::_NormalizedSIFT =2;
			break;  
        case MAKEINT1(b):
...
const char* SiftGPU::GetRendererInfo()
{
	if (GlobalUtil::_GoodOpenGL > 0)
...
void SiftGPU::GetFeatureVector(SiftKeypoint * keys, float * descriptors)
{
	...
	{
		_pyramid->CopyFeatureVector((float*)keys, descriptors);
	}else
	{
		_pyramid->CopyFeatureVector((float*)keys, (float*)NULL);
	}
}
void SiftGPU::GetFeatureVector(SiftKeypoint * keys, unsigned char * descriptors)
{
	if(GlobalUtil::_DescriptorPPT)
	{
		_pyramid->CopyFeatureVector((float*)keys, descriptors);
	...
	{
		_pyramid->CopyFeatureVector((float*)keys, (unsigned char*)NULL);
	}
}
