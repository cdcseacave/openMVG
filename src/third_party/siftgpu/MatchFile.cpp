...
void MatchFile::GetRecordList()
{
	...
	for(int i = 0; i < _match_records.size(); ++i)
	{
            ...
            (rp->fcount > 1000000 || rp->fcount < 0))
        {
            ...
            _header.definition_size = (int)(p - &_record_definition[0]);
            break;
        }else
        {
...
void MatchFile::GetMatchedImageList(std::vector<std::string>& paths)
{
	char match_path[MAX_PATH];
	GetMatchFolderPath(match_path);
	std::string folder = match_path;
	paths.resize(0);
...
void MatchFile::SaveSubsetMatch(std::vector<std::string>& paths, std::vector<int>&fc)
{
    MatchFile sub;
...
int  MatchFile::AddImageMatch(const char* relative_path)
{
	...
	int slen = ((int)strlen(relative_path) / 4  + 1) * 4; 
	int elen = slen - 4;
	...
	}else 
	{
		...
		std::vector<char> read_buffer(extra_space);
		...
		//organize file names
		const int definition_move = (int)((char*)_match_records[match_move_needed] - &_record_definition[0]);
		if(match_move_needed < _header.file_count)
		{
			//reorganize strings..move several string to the end
			std::vector<char> temp = _record_definition;
...
void MatchFile::CloseMatchFile()
{
	if(_fid > 0) 
	{
		if(_opened_for_write && delay_header_update)
...
void MatchFile::WriteIMatch(const char* image1, const char* image2, int NM, TwoViewGeometry& tvg, Points<int>& inliers, bool noReverse)
{
	{
	MatchFile file1;
	if(file1.OpenMatchFile(image1, 1))
							...
							TwoViewGeometry& tvg, Points<int>& inliers)
{
	...
	if(szmin > loc->block_size) //this is unlikely to happen, right?
	{
		//not enough size.
		...
		{
			printf("Reallocate space for record %d\r\n", index);
			UpdateHeaderAndRecordLoc();
			_lseek(_fid, loc->read_loc + sizeof(int), SEEK_SET);
		}
	}
...
void MatchFile::WritePMatch(const char* image1, const char* image2, 
							int fc1, int fc2, int NM, Points<int>& matches, bool noReverse)
{
	{
	MatchFile file1;
	if(file1.OpenMatchFile(image1, 1))
...
void MatchFile::UpdateHeaderAndRecordLoc()
{
	...
	if(_header.definition_size > (int)_recordloc_unchanged)
	{
		//recordloc
		_lseek(_fid, sizeof(_header) + _recordloc_unchanged, SEEK_SET);
		_write(_fid, &_record_definition[_recordloc_unchanged], _header.definition_size - (int)_recordloc_unchanged);
		_recordloc_unchanged = _header.definition_size;
	}
}
...
int MatchFile::GetPMatch(const char* image_path, int FC, int& NM, Points<int>& matches)
{
	...
	}else
	{
		...
		if(loc->fcount == FC || FC == -1)
		{
			_lseek(_fid, loc->read_loc, SEEK_SET);
			_read(_fid, &NM, sizeof(int));
			_lseek(_fid, sizeof(MatchRecordV3), SEEK_CUR);

			if(NM > 0 && NM <= loc->fcount)
			{
				matches.resize(NM, 2);
...
// same, but store reverse matches
