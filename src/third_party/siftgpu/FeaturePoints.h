...
public:
	...
	public:
	    int  _file_version;
	public:
		//each feature has x,y, color, size, orientation
		//for rect feature, there is x, y, width, height
		//for eclipse feature, there is u,v,a,b,c +z
	public:
...
protected:
	void ReadFeatureDataA(std::ifstream& is, int npt, int locDim, int desDim, int szFeatureName);
...
public:
	...
	void ResizeLocationData(int npoint, int locDim = 5)
	{
		if(npoint == 0)
		{
			if(_locData) delete _locData;
