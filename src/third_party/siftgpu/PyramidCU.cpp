...
#define LEVEL_FINISH()		if(GlobalUtil::_timingL){ ProgramCU::FinishCUDA(); tt = CLOCK(); LOG_OUT()<<(tt-t)<<"\t"; t = CLOCK(); }
...
{
	char name[2048];
	const int ret = ProgramCU::CheckCudaDevice(device, name);
	if (ret) {
		LOG_OUT() << "CUDA device: " << name << "\n";
	}else  
	{
		LOG_ERR() << "Unable to create CUDA device!\n";
	}
	return ret;
}
...
void PyramidCU::GetFeatureDescriptors() 
{
	...
	std::vector<float> descriptor_buffer2;
...
void PyramidCU::ReshapeFeatureListCPU() 
{
	...
	for(i = 0; i < n; i++)
	{
		...
		float * src = buffer1;
		float * des = buffer2;
		const static double factor = TWO_PI/65535.0;
		...
		for(int k = 0;  k < fcount; k++, ds+=4, fs+=4)
		{
			...
			ds[2] = oss*fs[2];  //scale
			ds[3] = (float)fmod(TWO_PI-fs[3], TWO_PI);	//orientation, mirrored
		}
#endif
		_featureNum += fcount;
	}
...
{
	//it is weired that this part is very slow.
		...
		for(int j = 0; j  < param._dog_level_num; j++, idx++, ftex++)
		{
			...
			{	
				...
				for(int k = 0;  k < _levelFeatureNum[idx]; k++, ps+=4)
				{
					...
					ps[2] = os*ps[2]; 
					ps[3] = (float)fmod(TWO_PI-ps[3], TWO_PI);	//orientation, mirrored
				}
...
void PyramidCU::GenerateFeatureList(int i, int j, int reduction_count, std::vector<int>& hbuffer)
{
...
void PyramidCU::ConvertInputToCU(GLTexInput* input)
{
    ...
    }else
    {
	    ...
	    {
			LOG_ERR()<<"Unable To Convert Intput\n";
	    }
    }
}
...
void PyramidCU::CopyGradientTex()
{
	...
	if(GlobalUtil::_timingS)
	{
		...
		{
			t1 = CLOCK();
			LOG_OUT()<<"<Copy Grad/Orientation>\t"<<(t1-ts)<<"\n";
		}
	}
}
