...
GLuint GlobalParam::	_texTarget = GL_TEXTURE_RECTANGLE; //only this one is supported
GLuint GlobalParam::	_iTexFormat =GL_RGBA32F; //or GL_RGBA16F
...
int GlobalParam::		_FeatureTexBlock = 4; //feature texture storage alignment
...
int	GlobalParam::       _NormalizedSIFT = 2;   //normalize descriptor
int GlobalParam::       _BinarySIFT = 0;       //saving binary format
int	GlobalParam::		_ExitAfterSIFT = 0;    //exit after saving result
...
void GlobalUtil::SetTextureParameterUS()
{
	GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
	GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
	GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_MAG_FILTER, GL_LINEAR)); 
	GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_MIN_FILTER, GL_NEAREST)); 
	GL_ASSERT(glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE));
}
...
{
	GLint port[4];
	GL_ASSERT(glGetIntegerv(GL_VIEWPORT, port));
	if(port[2] !=width || port[3] !=height)
	{
		...
		GL_ASSERT(glOrtho(0, width, 0, height, 0, 1));		
...
bool GlobalUtil::CheckFramebufferStatus() {
    GLenum status;
	GL_ASSERT(status=(GLenum)glCheckFramebufferStatus(GL_FRAMEBUFFER));
    switch(status) {
        case GL_FRAMEBUFFER_COMPLETE:
            return true;
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            LOG_ERR()<<("Framebuffer incomplete,incomplete attachment\n");
            return false;
        case GL_FRAMEBUFFER_UNSUPPORTED:
            LOG_ERR()<<("Unsupported framebuffer format\n");
            return false;
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            LOG_ERR()<<("Framebuffer incomplete,missing attachment\n");
            return false;
        case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
            LOG_ERR()<<("Framebuffer incomplete,attached images must have same dimensions\n");
            return false;
        case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
             LOG_ERR()<<("Framebuffer incomplete,attached images must have same format\n");
            return false;
        case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
            LOG_ERR()<<("Framebuffer incomplete,missing draw buffer\n");
            return false;
        case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
            LOG_ERR()<<("Framebuffer incomplete,missing read buffer\n");
            return false;
    }
	return false;
}
...
void ClockTimer::StopTimer(int verb)
{
	...
	if(verb && GlobalUtil::_verbose)
	{
		LOG_OUT()<<"["<<_current_event<<"]:\t"<<GetElapsedTime()<<"\n";
	}
}
...
void GlobalUtil::SetGLParam()
{
	...
	GL_ASSERT(glEnable(GlobalUtil::_texTarget));
	GL_ASSERT(glActiveTexture(GL_TEXTURE0));
}
    ...
    }else
    {
	    ...
	    if(vendor)
	    {
			...
			if(GlobalUtil::_IsNvidia && glewGetExtension("GL_NVX_gpu_memory_info"))
			{
				...
				_MemCapGPU /= (1024); 
				if(GlobalUtil::_verbose) LOG_OUT() << "[GPU VENDOR]:\t" << vendor << ' ' <<_MemCapGPU << "MB\n";
			...
			{
				if(GlobalUtil::_verbose) LOG_OUT() << "[GPU VENDOR]:\t" << vendor << "\n";
			}
	    }
	    ...
	    {
		    LOG_ERR() << "Shader not supported by your hardware!\n";
		    GlobalUtil::_GoodOpenGL = 0;
	    }

	    if (glewGetExtension("GL_EXT_framebuffer_object") != GL_TRUE) 
	    {
		    LOG_ERR() << "Framebuffer object not supported!\n";
		    GlobalUtil::_GoodOpenGL = 0;
	    }
	    ...
	    {
	        GLint value;
		    GlobalUtil::_texTarget = GL_TEXTURE_RECTANGLE;
		    glGetIntegerv(GL_MAX_RECTANGLE_TEXTURE_SIZE, &value);
		    GlobalUtil::_texMaxDimGL = value; 
		    if(GlobalUtil::_verbose) LOG_OUT() << "TEXTURE:\t" << GlobalUtil::_texMaxDimGL << "\n";
	    ...
	    {
		    LOG_ERR() << "GL_ARB_texture_rectangle not supported!\n";
		    GlobalUtil::_GoodOpenGL = 0;
	    }
	    ...
	    GL_ASSERT(glPolygonMode(GL_FRONT, GL_FILL));
...
void GlobalUtil::SelectDisplay()
{
	...
	if(hdc == NULL) 
	{
		LOG_OUT() << "ERROR: invalid display specified\n";
		return;
	}
	...
	if(ChoosePixelFormat(hdc, &pfd) == 0) 
	{
		LOG_OUT() << "ERROR: invalid pixel format\n";
		return;
	}
#endif
}
...
int GlobalUtil::CreateWindowEZ(LiteWindow* window)
{
    ...
    {
        window->MakeCurrent();
		LOG_OUT() << "OpenGL device: " << glGetString(GL_RENDERER) << "\n";
        return 1;
    }
    else  
    {
        LOG_ERR() << "Unable to create OpenGL Context!\n";
		#ifdef CUDA_SIFTGPU_ENABLED
		LOG_ERR() << "For nVidia cards, you can try change to CUDA mode in this case\n";
		#endif
        return 0;
    }
}
