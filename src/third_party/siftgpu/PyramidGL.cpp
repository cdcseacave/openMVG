...
#define OCTAVE_START()		if(GlobalUtil::_timingO){t = t0 = CLOCK(); LOG_OUT()<<"#"<<i+_down_sample_factor<<"\t";}
#define LEVEL_FINISH()		if(GlobalUtil::_timingL){GL_ASSERT(glFinish()); tt = CLOCK(); LOG_OUT()<<(tt-t)<<"\t"; t = CLOCK();}
#define OCTAVE_FINISH()		if(GlobalUtil::_timingO){LOG_OUT()<<"|\t"<<(CLOCK()-t0)<<std::endl;}
...
void PyramidGL::DestroyPerLevelData()
{
	...
	{
		GL_ASSERT(glDeleteBuffers(no, _featureDisplayVBO));
		delete [] _featureDisplayVBO;
		_featureDisplayVBO = NULL;
	}
	if(_featurePointVBO)
	{
		GL_ASSERT(glDeleteBuffers(no, _featurePointVBO));
...
void PyramidNaive::InitPyramid(int w, int h, int ds)
{
	...
	{
		LOG_OUT()<<(toobig == 2 ? "[**SKIP OCTAVES**]:\tExceeding Memory Cap (-nomc)\n" :
...
void PyramidNaive::ResizePyramid(int w, int h)
{
	...
	if(GlobalUtil::_verbose && GlobalUtil::_timingS) LOG_OUT()<<"[Allocate Pyramid]:\t" <<w<<"x"<<h<<std::endl;
	...
	if(GlobalUtil::_verbose && GlobalUtil::_timingS) LOG_OUT()<<"[Allocate Pyramid]:\t" <<(totalkb/1024)<<"MB\n";
...
int PyramidGL::ResizeFeatureStorage()
{
	...
	if(_descriptorTex==NULL)
	{
		...
		{
			//at least one, reuse for visualization and other work
			nf = 1; ns = 4;
		}
		//
		_alignment = ns;
...
void PyramidNaive::BuildPyramid(GLTexInput *input)
{
	...
	if(GlobalUtil::_timingS) GL_ASSERT(glFinish());
	UnloadProgram();
}
...
void PyramidNaive::ComputeGradient()
{
	...
	if(GlobalUtil::_timingS && GlobalUtil::_verbose)
	{
		GL_ASSERT(glFinish());
		t1 = CLOCK();	
		LOG_OUT()<<"<Compute Gradient>\t"<<(t1-ts)<<"\n";
	}

	UnloadProgram();
	GLTexImage::UnbindMultiTex(3);
	fbo.UnattachTex(GL_COLOR_ATTACHMENT1);
}


//keypoint detection with sub-pixel localization
void PyramidNaive::DetectKeypointsEX()
{
	...
	for (i = _octave_min; i < _octave_min + _octave_num; i++)
	{
		...
		for(j = param._level_min + 2; j <  param._level_max; j++, aux++, tex++)
		{
			...
			(tex-1)->BindTex();
			ShaderMan::UseShaderKeypoint((tex+1)->GetTexID(), (tex-1)->GetTexID(), param._dog_threshold, param._edge_threshold);
			aux->DrawQuadMT8();
...
void PyramidNaive::GenerateFeatureList(int i, int j)
{
	...
	{	
		//read back one pixel
		float fn[4], fcount;
		GL_ASSERT(glReadPixels(0, 0, 1, 1, GL_RGBA , GL_FLOAT, fn));
	...
	}else
	{
		...
		int fc = 0;
		GL_ASSERT(glReadPixels(0, 0, tw, th, GL_RGBA , GL_FLOAT, _histo_buffer));	
		...
		GL_ASSERT(glActiveTexture(GL_TEXTURE0));
		ftex->BindTex();
		GL_ASSERT(glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, ftex->GetDrawWidth(), ftex->GetDrawHeight(), GL_RGBA, GL_FLOAT, &_keypoint_buffer[0]));
		htex += 2;
	}
...
void PyramidNaive::GenerateFeatureList()
{
    ...
    FOR_EACH_OCTAVE(i, reverse)
	{
		...
		if(GlobalUtil::_timingO)
		{
            t1= CLOCK();
			ocount = 0;
			LOG_OUT()<<"#"<<i+_octave_min + _down_sample_factor<<":\t";
		}
		//for(int j = 0; j < param._dog_level_num; j++, idx++)
		...
		{	
			t2 = CLOCK(); 
			LOG_OUT() << "| \t" << int(ocount) << " :\t(" << (t2 - t1) << ")\n";
		}
	}
	if(GlobalUtil::_timingS) GL_ASSERT(glFinish());
	if(GlobalUtil::_verbose)
	{
		LOG_OUT()<<"#Features:\t"<<_featureNum<<"\n";
	}
}
...
void PyramidGL::GenerateFeatureDisplayVBO()
{
	...
	if(_featureDisplayVBO==NULL)
	{
		...
		GL_ASSERT(glGenBuffers(nvbo, _featureDisplayVBO));
    }
    if(_featurePointVBO == NULL)
    {
		_featurePointVBO = new GLuint[nvbo];
		GL_ASSERT(glGenBuffers(nvbo, _featurePointVBO));
	}
	...
	for(int i = 0, idx = 0; i < _octave_num; i++)
	{
		for(int j = 0; j < param._dog_level_num; j ++, idx++)
		{
			...
			GL_ASSERT(glBindBuffer(GL_PIXEL_PACK_BUFFER, _featurePointVBO[ idx]));
			GL_ASSERT(glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize));
			esize = ftex->GetImgHeight() * ftex->GetImgWidth()*sizeof(float) *4;

			//increase size when necessary
			if(bsize < esize)
			{
				GL_ASSERT(glBufferData(GL_PIXEL_PACK_BUFFER, esize*3/2, NULL, GL_STATIC_DRAW));
				GL_ASSERT(glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize));
			}

			//read back if we have enough buffer
			if(bsize >= esize) GL_ASSERT(glReadPixels(0, 0, ftex->GetImgWidth(), ftex->GetImgHeight(), GL_RGBA, GL_FLOAT, 0))
			else GL_ASSERT(glBufferData(GL_PIXEL_PACK_BUFFER, 0, NULL, GL_STATIC_DRAW));
			GL_ASSERT(glBindBuffer(GL_PIXEL_PACK_BUFFER, 0));
			...
			//
			GL_ASSERT(glBindBuffer(GL_PIXEL_PACK_BUFFER, _featureDisplayVBO[idx]));
			GL_ASSERT(glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize));
			...
			//read back if we have enough buffer	
			if(bsize >= esize) GL_ASSERT(glReadPixels(0, 0, w, h, GL_RGBA, GL_FLOAT, 0))
			else GL_ASSERT(glBufferData(GL_PIXEL_PACK_BUFFER, 0, NULL, GL_STATIC_DRAW));
			GL_ASSERT(glBindBuffer(GL_PIXEL_PACK_BUFFER, 0));
		}
	}
...
void PyramidNaive::GetFeatureOrientations()
{
	...
	if(_orientationTex)	fbo.UnattachTex(GL_COLOR_ATTACHMENT1);
}
...
void PyramidNaive::GenerateFeatureListCPU()
{
	...
	for(int i = 0; i < _octave_num; i++)
	{
		for(int j = 0; j < param._dog_level_num; j++, idx++)
		{
			...
			tex->BindTex();
			GL_ASSERT(glGetTexImage(GlobalUtil::_texTarget, 0, GL_RED, GL_FLOAT, mem));
			//tex->AttachToFBO(0);
			//tex->FitTexViewPort();
			//GL_ASSERT(glReadPixels(0, 0, tex->GetTexWidth(), tex->GetTexHeight(), GL_RED, GL_FLOAT, mem));
			//
			//make a list of 
			list.resize(0);
			float * p = mem;
			...
			ftex->BindTex();
			ftex->AttachToFBO(0);
			//GL_ASSERT(glTexImage2D(GlobalUtil::_texTarget, 0, GlobalUtil::_iTexFormat, fw, fh, 0, GL_BGRA, GL_FLOAT, &list[0]));
			GL_ASSERT(glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, fw, fh, GL_RGBA, GL_FLOAT, &list[0]));
		}
	}
...
void PyramidGL::ReshapeFeatureListCPU()
{
	...
	//download orientations and the feature list
    ...
    GL_ASSERT(glGenBuffers(1, &ListUploadPBO)); 
	GL_ASSERT(glBindBuffer(GL_PIXEL_UNPACK_BUFFER, ListUploadPBO));
	GL_ASSERT(glBufferData(GL_PIXEL_UNPACK_BUFFER, szmax * 8 * sizeof(float), NULL, GL_STREAM_DRAW));
#endif
	...
	for(i = 0; i < n; i++)
	{
		...
		GL_ASSERT(glReadPixels(0, 0, _featureTex[i].GetImgWidth(), _featureTex[i].GetImgHeight(),GL_RGBA, GL_FLOAT, buffer1));
		...
		{	
			...
			GL_ASSERT(glReadPixels(0, 0, _orientationTex[i].GetImgWidth(), _orientationTex[i].GetImgHeight(), GL_RGBA, GL_FLOAT, buffer2));
		...
		}else
		{
			...
			const static float factor((float)(TWO_PI/65535.0));
		...
		GL_ASSERT(glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, nfw, nfh, GL_RGBA, GL_FLOAT, buffer3));
		_featureTex[i].UnbindTex();
#else
        float* mem; GL_ASSERT(mem = (float*)glMapBuffer(GL_PIXEL_UNPACK_BUFFER, GL_WRITE_ONLY));
        memcpy(mem, buffer3, sz * 4 * sizeof(float));
		GL_ASSERT(glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER));
		_featureTex[i].BindTex();
		GL_ASSERT(glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, nfw, nfh, GL_RGBA, GL_FLOAT, NULL));
		_featureTex[i].UnbindTex();
#endif
		...
		if(fcount > 0)
		{
			...
			for(int k = 0;  k < fcount; k++, ds+=4, fs+=4)
			{
				...
				ds[3] = fmod(FTWO_PI-fs[2], FTWO_PI); //orientation, mirrored
				ds[2] = oss*fs[3]; //scale
			}
		}
#endif
	...
	if(GlobalUtil::_verbose)
	{
		LOG_OUT()<<"#Features MO:\t"<<_featureNum<<std::endl;
	}
    ///////////////////////////////////
#ifdef FEATURELIST_USE_PBO
	GL_ASSERT(glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0));
    GL_ASSERT(glDeleteBuffers(1, &ListUploadPBO));
#endif
}
...
inline void PyramidGL::RootNormalizeDescriptor(int num, float* pd)
{
	typedef Eigen::Matrix<float,128,1> Vector128;
	for (float* pdend=pd+num*128; pd!=pdend; pd+=128) {
		Eigen::Map<Vector128> desc(pd);
		// normalize and truncate to .2
		const float nrm = 1.f/desc.norm();
		#ifdef _MSC_VER
		multiply_and_truncate_128d(pd, nrm);
		#else
		for (float* ppd = pd, *ppde = pd+128; ppd++ < ppde; )
			*ppd = min(*ppd*nrm, 0.2f);
		#endif
		// L1-normalize as in RootSIFT:
		// rootsift = sqrt(sift / sum(sift));
		// (see "Three things everyone should know to improve object retrieval", R. Arandjelovic, A. Zisserman, CVPR2012.
		desc = (desc*(1.f/desc.sum())).cwiseSqrt();
	}
}
...
void PyramidGL::GetFeatureDescriptors()
{
	//descriptors...
	int idx, i, j, k, w, h;
	const int ndf = 32 / GlobalUtil::_DescriptorPPT; //number of textures
	const int block_width = GlobalUtil::_DescriptorPPR;
	const int block_height = GlobalUtil::_DescriptorPPT/GlobalUtil::_DescriptorPPR;
	float *pd = &_descriptor_buffer[0], *pbuf = NULL;
	std::vector<float>read_buffer, descriptor_buffer2;
	...
	GL_ASSERT(glDrawBuffers(ndf, buffers));
	GL_ASSERT(glReadBuffer(GL_COLOR_ATTACHMENT0));
	...
	for(i = 0, idx = 0, ftex = _featureTex; i < _octave_num; i++)
	{
		...
		for(j = 0; j < param._dog_level_num; j++, ftex++, idx++, gtex++, otex++)
		{
            ...
            const float sigma(IsUsingRectDescription() ? 0 : param.GetLevelSigma(j+param._level_min+1));
			...
			//not enough space for holding the descriptor data
			...
			{
				GL_ASSERT(glReadBuffer(GL_COLOR_ATTACHMENT0 + k));
				if(GlobalUtil::_IsNvidia ||  w * h <=  16384) //were
				{
					GL_ASSERT(glReadPixels(0, 0, w, h, GL_RGBA, GL_FLOAT, pbuf));
				}else
				{
					int hstep = 16384 / w; 
					for(int kk = 0; kk < h; kk += hstep)
						GL_ASSERT(glReadPixels(0, kk, w, min(hstep, h - kk), GL_RGBA, GL_FLOAT, pbuf + w * kk * 4));
				}
			}
			...
			switch (GlobalUtil::_NormalizedSIFT) {
			case 1:
				NormalizeDescriptor(_levelFeatureNum[idx], pd);
				break;
			case 2:
				RootNormalizeDescriptor(_levelFeatureNum[idx], pd);
				break;
			}
	...
	if(_keypoint_index.size() > 0)
	{
		for(i = 0; i < _featureNum; ++i)
		{
			...
			memcpy(&_descriptor_buffer[index*128], &descriptor_buffer2[i*128], sizeof(float) * 128);
		}
	}
	...
	ShaderMan::UnloadProgram();
	if(GlobalUtil::_timingS) GL_ASSERT(glFinish());
	for(i = 0; i < ndf; i++) fbo.UnattachTex(GL_COLOR_ATTACHMENT0 +i);
}
...
void PyramidGL::DownloadKeypoints()
{
	...
	std::vector<float> keypoint_buffer2;
	//use a different keypoint buffer when processing with an existent features list
	...
	for(int i = 0; i < _octave_num; i++, os *= 2.0f)
	{
		for(int j = 0; j  < param._dog_level_num; j++, idx++, ftex++)
		{
			...
			{	
				ftex->AttachToFBO(0);
				GL_ASSERT(glReadPixels(0, 0, ftex->GetImgWidth(), ftex->GetImgHeight(),GL_RGBA, GL_FLOAT, p));
				ps = p;
				for(int k = 0;  k < _levelFeatureNum[idx]; k++, ps+=4)
				{
					...
					sigma = os*ps[3]; 
					ps[3] = fmod(FTWO_PI-ps[2], FTWO_PI);	//orientation, mirrored
					ps[2] = sigma;  //scale
				}
		...
		for(int j = 0; j < param._dog_level_num; j++, idx++)
		{
			...
			for(int k = 0; k < _featureNum; k++)
			{
                            ...
                            (i == _octave_num -1  || GlobalUtil::_KeyPointListForceLevel0)))
				{
                    ...
                    {
					    list.push_back((float)fmod(TWO_PI-key[3], TWO_PI));
					    list.push_back(key[2] / octave_sigma);
                    }
					fcount++;
...
void PyramidPacked::BuildPyramid(GLTexInput * input)
{
	...
	{
		tex = GetBaseLevel(i);
		tmp = GetBaseLevel(i, DATA_DOG) + 2; //use this as a temporary texture
...
void PyramidPacked::DetectKeypointsEX()
{
	...
	for (i = _octave_min; i < _octave_min + _octave_num; i++)
	{
		...
		for(j = param._level_min +2; j <  param._level_max; j++, dog++, key++, gus++)
		{
			...
			ShaderMan::UseShaderKeypoint((dog+1)->GetTexID(), (dog-1)->GetTexID(), param._dog_threshold, param._edge_threshold);
			key->DrawQuadMT8();
			if(GlobalUtil::_timingL)
			{
				GL_ASSERT(glFinish());
...
void PyramidPacked::GenerateFeatureListCPU()
{
	...
	for(int i = 0; i < _octave_num; i++)
	{
		for(int j = 0; j < param._dog_level_num; j++, idx++)
		{
			tex = GetBaseLevel(_octave_min + i, DATA_KEYPOINT) + j + 2;
			tex->BindTex();
			GL_ASSERT(glGetTexImage(GlobalUtil::_texTarget, 0, GL_RGBA, GL_FLOAT, mem));
			//tex->AttachToFBO(0);
			//tex->FitTexViewPort();
			//GL_ASSERT(glReadPixels(0, 0, tex->GetTexWidth(), tex->GetTexHeight(), GL_RED, GL_FLOAT, mem));
			//
			...
			if(GlobalUtil::_timingL) LOG_OUT()<<fcount<<".";
...
void PyramidPacked::ResizePyramid(int w, int h)
{
	...
	for(i = 0; i< noct; i++)
	{
		...
		//several auxiliary textures are not actually required
		w>>=1;
		h>>=1;
	}
