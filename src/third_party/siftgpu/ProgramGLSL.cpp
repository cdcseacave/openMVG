...
ProgramGLSL::ShaderObject::ShaderObject(int shadertype, const char * source, int filesource)
{
	...
	GL_ASSERT(_shaderID = glCreateShader(shadertype));
	...
	if(source)
	{
		...
		if(filesource == 0)
		{
			...
			GL_ASSERT(glShaderSource(_shaderID, 1, (const char **) &code, &code_length));
		}else
		{
			char * code;
		...
		GL_ASSERT(glCompileShader(_shaderID));

		CheckCompileLog();

		if(!_compiled) LOG_OUT() << source << std::endl;
	}
}
...
{
	GLint status;
	GL_ASSERT(glGetShaderiv(_shaderID, GL_COMPILE_STATUS, &status));
...
void ProgramGLSL::ShaderObject::PrintCompileLog(std::ostream&os)
{
	GLint len = 0;	
...
ProgramGLSL::ProgramGLSL()
{
	_linked = 0;
	_TextureParam0 = -1;
	GL_ASSERT(_programID = glCreateProgram());
}
ProgramGLSL::~ProgramGLSL()
{
	if(_programID)
...
void ProgramGLSL::AttachShaderObject(ShaderObject &shader)
{
	if(_programID  && shader.IsValidShaderObject()) 
		GL_ASSERT(glAttachShader(_programID, shader.GetShaderID()));
}
void ProgramGLSL::DetachShaderObject(ShaderObject &shader)
{
	if(_programID  && shader.IsValidShaderObject()) 
		GL_ASSERT(glDetachShader(_programID, shader.GetShaderID()));
}
int ProgramGLSL::LinkProgram()
{
	...
	GL_ASSERT(glLinkProgram(_programID));
...
int ProgramGLSL::UseProgram()
{
	if(ValidateProgram())
	{
		GL_ASSERT(glUseProgram(_programID));
		if (_TextureParam0 >= 0) GL_ASSERT(glUniform1i(_TextureParam0, 0));
		return true;
	}
	return false;
}
...
ProgramGLSL::ProgramGLSL(const char *frag_source)
{
	...
	if(shader.IsValidFragmentShader())
	{
		...
		{
			GL_ASSERT(_TextureParam0 = glGetUniformLocation(_programID, "tex"));
		}
	}else
	{
		_linked = 0;
...
ProgramGLSL::ProgramGLSL(char*frag_source, char * vert_source)
{
	...
	if(!_linked)
	{
		shader.PrintCompileLog(LOG_OUT());
		vertex_shader.PrintCompileLog(LOG_OUT());
		PrintLinkLog(LOG_OUT());
		LOG_OUT()<<vert_source;
		LOG_OUT()<<frag_source;
	}
...
FilterGLSL::FilterGLSL(float sigma) 
{
	...
	{
		LOG_OUT()<<"Filter size truncated from "<<width<<" to "<<GlobalUtil::_MaxFilterWidth<<std::endl;
	...
	delete[] kernel; 
    if(GlobalUtil::_verbose && GlobalUtil::_timingL) LOG_OUT()<<"Filter: sigma = "<<sigma<<", size = "<<width<<"x"<<width<<std::endl;
}
...
ShaderBag::~ShaderBag()
{
	...
	if(s_copy_key)delete s_copy_key;
...
void ShaderBagGLSL::LoadFixedShaders()
{
	...
	"	gl_FragColor.a = size;}");  
		...
		LOG_ERR()<<"Orientation simplified on this hardware"<<std::endl;
	}
	...
	if(s_descriptor_fp == NULL) 
	{
		...
		LOG_ERR()<<"Descriptor ignored on this hardware"<<std::endl;
	}
...
void ShaderBagGLSL::LoadKeypointShader(float threshold, float edge_threshold)
{
	...
	out <<
	"uniform float threshold0, threshold1, threshold2;\n";
	...
	}else
	{
		...
		LOG_ERR()<<"Detection simplified on this hardware"<<std::endl;
	}

	_param_dog_texu = glGetUniformLocation(*program, "texU");
	_param_dog_texd = glGetUniformLocation(*program, "texD");
	_param_dog_threshold0 = glGetUniformLocation(*program, "threshold0");
	_param_dog_threshold1 = glGetUniformLocation(*program, "threshold1");
	_param_dog_threshold2 = glGetUniformLocation(*program, "threshold2");
}
...
void ShaderBagGLSL::SetDogThreshold(float dog_threshold, float edge_threshold)
{
	glUniform1f(_param_dog_threshold0, dog_threshold * (GlobalUtil::_SubpixelLocalization?0.8f:1.0f));
	glUniform1f(_param_dog_threshold1, dog_threshold);
	glUniform1f(_param_dog_threshold2, (edge_threshold+1)*(edge_threshold+1)/edge_threshold);
}
...
void ShaderBagPKSL::SetDogThreshold(float dog_threshold, float edge_threshold)
{
	glUniform1f(_param_dog_threshold0, dog_threshold * (GlobalUtil::_SubpixelLocalization?0.8f:1.0f));
	glUniform1f(_param_dog_threshold1, dog_threshold);
	glUniform1f(_param_dog_threshold2, (edge_threshold+1)*(edge_threshold+1)/edge_threshold);
