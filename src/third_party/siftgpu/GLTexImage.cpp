...
int GLTexImage::CheckTexture()
{
	if(_texID)
	{
		GLint tw, th;
		BindTex();
		GL_ASSERT(glGetTexLevelParameteriv(_texTarget, 0, GL_TEXTURE_WIDTH, &tw));
		GL_ASSERT(glGetTexLevelParameteriv(_texTarget, 0, GL_TEXTURE_HEIGHT, &th));
		UnbindTex();
...
{
	InitTexture(width, height, clamp_to_edge, _iTexFormat);
}
void GLTexImage::InitTexture(int width, int height, int clamp_to_edge, GLuint format)
{
	...
	if(_texID==0) GL_ASSERT(glGenTextures(1, &_texID)); 
	...
	if(clamp_to_edge)
	{
		GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE)); 
		GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE)); 
	}else
	{
		//out of bound tex read returns 0??
		GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER)); 
		GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER)); 
	}
	GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_MAG_FILTER, GL_NEAREST)); 
	GL_ASSERT(glTexParameteri(_texTarget, GL_TEXTURE_MIN_FILTER, GL_NEAREST)); 
	GL_ASSERT(glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE));

	GL_ASSERT(glTexImage2D(_texTarget, 0, format, _texWidth, _texHeight, 0, GL_RGBA, GL_FLOAT, NULL)); 
...
{
	GL_ASSERT(glBindTexture(_texTarget, _texID));
}

void  GLTexImage::UnbindTex()
{
	GL_ASSERT(glBindTexture(_texTarget, 0));
}
...
{
	GL_ASSERT(glFramebufferTexture2D(GL_FRAMEBUFFER, i+GL_COLOR_ATTACHMENT0, _texTarget, _texID, 0));
}

void  GLTexImage::DetachFBO(int i)
{
	GL_ASSERT(glFramebufferTexture2D(GL_FRAMEBUFFER, i+GL_COLOR_ATTACHMENT0, _texTarget, 0, 0));
}
...
{
	//update the actual size of draw area
	_drawWidth  = (1 + _imgWidth ) >> 1;
	_drawHeight = (1 + _imgHeight) >> 1;
	///
	FrameBufferObject fbo;
	GLuint oldTexID = _texID;
	GL_ASSERT(glGenTextures(1, &_texID));
	GL_ASSERT(glBindTexture(_texTarget, _texID));
	GL_ASSERT(glTexImage2D(_texTarget, 0, _iTexFormat, _texWidth, _texHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL));

	//input
	GL_ASSERT(glBindTexture(_texTarget, oldTexID));
	//output 
	AttachToFBO(0);
	//program
	ShaderMan::UseShaderRGB2Gray();
	...
	GL_ASSERT(glDeleteTextures(1, &oldTexID));
	DetachFBO(0);
}
...
void GLTexPacked::InitTexture(int width, int height, int clamp_to_edge)
{
	...
	GL_ASSERT(glTexImage2D(_texTarget, 0, _iTexFormat, _texWidth, _texHeight, 0, GL_RGBA, GL_FLOAT, NULL)); 
...
GLTexInput::
#endif
DownSamplePixelDataI(unsigned int gl_format, int width, int height, int ds, const Uint * pin, Uint * pout)	
{
	int step, linestep;
	int i, j; 
	int ws = width/ds;
...
int GLTexInput::SetImageData(int width, int height, const void * data, unsigned int gl_format, unsigned int gl_type)
{
	...
	if (ws > _texMaxDim || hs > _texMaxDim)
	{
		if(simple_format)
		{
			if(GlobalUtil::_verbose) LOG_OUT()<<"Automatic down-sampling is used\n";
			do
			{
				_down_sampled ++;
				ws >>= 1;
		...
		{
			LOG_ERR()<<"Input images is too big to fit into a texture\n";
			return 0;
		}
	}
	...
	{
		LOG_OUT()<<"Image size :\t"<<width<<"x"<<height<<"\n";
		if(_down_sampled >0) LOG_OUT()<<"Down sample to \t"<<ws<<"x"<<hs<<"\n";
	}
    ...
    if(GlobalUtil::_UseCUDA || GlobalUtil::_UseOpenCL)
    {
        ...
        if(!simple_format) 
        {
            LOG_ERR()<<"Input format not supported under current settings.\n";
            return 0;
        }else if(_down_sampled > 0 || gl_format != GL_LUMINANCE || gl_type != GL_FLOAT)
        {
            ...
            _rgb_converted = 2; //indicates a new data copy
    ...
    }else
    {
	    ...
	    if(simple_format && (_down_sampled > 0 || (gl_format != GL_LUMINANCE && GlobalUtil::_PreProcessOnCPU)))
	    {
		    if(gl_type == GL_UNSIGNED_BYTE)
		    {
				...
				GL_ASSERT(glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, newdata));
				delete[] newdata;
		    }else if(gl_type == GL_UNSIGNED_SHORT)
		    {
				...
				GL_ASSERT(glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_UNSIGNED_SHORT, newdata));
			    delete[] newdata;
		    }else if(gl_type == GL_FLOAT)
		    {
				...
				GL_ASSERT(glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_FLOAT, newdata));
			    delete[] newdata;
		    }else
		    {
			    //impossible
	    ...
	    }else
	    {
		    ...
		    if(gl_format == GL_LUMINANCE || gl_format == GL_LUMINANCE_ALPHA)
            {
				...
				GL_ASSERT(glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth, _imgHeight, 0, gl_format,	gl_type, data)); 
			    GlobalUtil::FitViewPort(1, 1); //this used to be necessary
            }
		    else
            {
	    	    //convert RGB 2 GRAY if needed
				GL_ASSERT(glTexImage2D(_texTarget, 0, _iTexFormat, _imgWidth, _imgHeight, 0, gl_format, gl_type, data)); 
...
int GLTexInput::LoadImageFile(char *imagepath, int &w, int &h)
{
	...
	if(ilLoadImage(imagepath))
	{
		...
		{
			LOG_OUT()<<"Image loaded :\t"<<imagepath<<"\n";
		}

	}else
	{
		LOG_ERR()<<"Unable to open image [code = "<<ilGetError()<<"]\n";
		done = 0;
	}
	...
	{
		fclose(file);
        LOG_ERR() << "ERROR: file format not supported\n";
		return 0;
	}else
    {
        w = width;
	...
	fclose(file);
    delete data; 
    if(GlobalUtil::_verbose && done) LOG_OUT() << "Image loaded :\t" << imagepath << "\n";
	return 1;
#endif
}
...
int GLTexImage::CopyToPBO(GLuint pbo, int width, int height, GLenum format)
{
	...
	AttachToFBO(0);
	GL_ASSERT(glBindBuffer(GL_PIXEL_PACK_BUFFER, pbo));
	GL_ASSERT(glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize));
	if(bsize < esize) 
	{
		GL_ASSERT(glBufferData(GL_PIXEL_PACK_BUFFER, esize,	NULL, GL_STATIC_DRAW));
		GL_ASSERT(glGetBufferParameteriv(GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE, &bsize));
	}
	if(bsize >= esize)
	{
		GL_ASSERT(glReadPixels(0, 0, width, height, format, GL_FLOAT, 0));
	}
...
void GLTexInput::VerifyTexture()
{
    ...
    BindTex();
	GL_ASSERT(glTexImage2D(_texTarget, 0, GL_LUMINANCE32F_ARB, _imgWidth, _imgHeight, 0, GL_LUMINANCE, GL_FLOAT, _pixel_data));
    UnbindTex();
    _data_modified = 0; 
}
...
void GLTexImage::CopyFromPBO(GLuint pbo, int width, int height, GLenum format)
{
	...
	{
		BindTex();
		GL_ASSERT(glBindBuffer(GL_PIXEL_UNPACK_BUFFER, pbo));
		GL_ASSERT(glTexSubImage2D(GlobalUtil::_texTarget, 0, 0, 0, width, height, format, GL_FLOAT, 0));
