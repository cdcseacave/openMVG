...
void SiftPyramid::RunSIFT(GLTexInput* input)
{
	...
	if(_existing_keypoints & SIFT_SKIP_DETECTION)
	{
	...
	{
		//use existing feature orientation or
	...
	}else
	{
		...
		if(GlobalUtil::_MaxOrientation < 2 || GlobalUtil::_FixedOrientation || (_existing_keypoints & (SIFT_SKIP_DETECTION|SIFT_SKIP_ORIENTATION)) == SIFT_SKIP_DETECTION)
#endif
		DownloadKeypoints();
		GlobalUtil::StopTimer();
	...
	{
		//descriptors are downloaded in descriptor computation of each level
...
void SiftPyramid::LimitFeatureCount(int have_keylist)
{
    ...
    if(GlobalUtil::_TruncateMethod == 2)
    {
        ...
        if(new_feature_num < _featureNum)
        {
            ...
            {
	            LOG_OUT()<<"#Features Reduced:\t"<<_featureNum<<std::endl;
            }
        }
    }else
    {
...
inline void QuantizeDescriptor(int num, const float* pd, unsigned char* qd)
{
	for (const float* pdend=pd+num*128; pd!=pdend;)
		for (unsigned char* qdend=qd+128; qd!=qdend; ++qd, ++pd)
			*qd = static_cast<unsigned char>(std::min(static_cast<int>(std::round(512.f*(*pd))), 255));
}
void SiftPyramid::CopyFeatureVector(float *keys, unsigned char *descriptors)
{
	if(keys)		memcpy(keys, &_keypoint_buffer[0], sizeof(float)*4*_featureNum);
	if(descriptors)	QuantizeDescriptor(_featureNum, &_descriptor_buffer[0], descriptors);
}
	...
	}else
	{
		...
		if(GlobalUtil::_DescriptorPPT)
		{
			...
			for(int i = 0; i < _featureNum; i++)
			{
				...
				////out << std::setprecision(12) << pk[1] <<  " " << pk[0] << " " << pk[2] << " " << pk[3] << std::endl;
				pk+=4;
				for(int k = 0; k < 128; k ++, pd++) 
				{
					...
					if ((k+1)%20 == 0) out<<std::endl; //suggested by Martin Schneider
...
void SiftPyramid::WriteTextureForDEBUG(GLTexImage * tex, const char *namet, ...)
{
	...
	*p++ = '/';
	...
	tex->AttachToFBO(0);
	tex->FitTexViewPort();
	GL_ASSERT(glReadPixels(0, 0, width, height, GL_RGBA , GL_FLOAT, buffer1));
