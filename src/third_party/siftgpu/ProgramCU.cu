...
int ProgramCU::CheckCudaDevice(int device, char* name)
{
    ...
    if (name)
    {
        ...
        sprintf(name, "%s (ID %d; compute capability %d.%d; memory %.2fGB; peak memory bandwidth %.2fGB/s)",
            prop.name, device, prop.major, prop.minor, (double)prop.totalGlobalMem/(1024*1024*1024), 2.0*prop.memoryClockRate*(prop.memoryBusWidth/8)/1.0e6
        );
    }
    return 1;
}
