...
public:
	enum
	{
		...
		SIFTGPU_PARTIAL_SUPPORTED = 1, // detection works, but not orientation/descriptor
...
public:
	...
	SIFTGPU_EXPORT virtual void GetFeatureVector(SiftKeypoint * keys, unsigned char * descriptors);
	...
	//retrieve OpenGL renderer info
	SIFTGPU_EXPORT virtual const char* GetRendererInfo();
	...
	//set a new image given the pixel data and format/type;
	...
	SIFTGPU_EXPORT virtual int  SetSIFT(int width, int height,	const void * data, 
	...
	SIFTGPU_EXPORT virtual int  RunSIFT(int width, int height,	const void * data, 
										unsigned int gl_format, unsigned int gl_type);
	//run SIFT with keypoints on a new image given the pixel data and format/type;
	SIFTGPU_EXPORT virtual int  RunSIFT(int width, int height,	const void * data, 
										unsigned int gl_format, unsigned int gl_type,
										int num, const SiftKeypoint * keys, int keys_have_orientation = 1);
	//run SIFT with keypoints on current image
	SIFTGPU_EXPORT virtual int  RunSIFT(int num, const SiftKeypoint * keys, int keys_have_orientation = 1, int run_on_current = 1);
...
class SiftGPUEX:public SiftGPU
{
	...
	//the first nskip1 levels and the last nskip2 levels are skipped in display
...
public:
	...
	//Constructor, the argument specifies the maximum number of features to match
	...
	//Specify descriptors to match, index = [0/1] for two features sets respectively
	...
	//two functions for guided matching, two constraints can be used 
...
//otherwise, 
//			Assumes the existence of a remote server and connects to it
...
//  // GPU-selection is done thought siftgpu->ParseParam, 
//  // it doesn't really initialize SiftGPU until you call CreateContextGL/VerifyContextGL
//  delete combo;
#endif 
