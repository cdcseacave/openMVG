...
public:
    ...
    void Create(int x = -1, int y = -1, const char* display = NULL)
    {
        ...
        if(display || x != -1) LOG_OUT() << "Using display ["
...
public:
    ...
    void Create(int x = -1, int y = -1, const char* display = NULL)
    {
		...
		if(hContext == NULL) 
			LOG_OUT() << "ERROR: failed creating the OpenGL context (" << GetLastError() << ")\n";
    }
    void MakeCurrent()
    {
        if(wglMakeCurrent(hdc, hContext) == FALSE)
			LOG_OUT() << "ERROR: failed making current the OpenGL context (" << GetLastError() << ")\n";
    }
};

#elif defined(_USE_OSMESA)

// probably a safer way todo this but glew undefs GLAPI blindly and osmesa needs it
#  define GLAPI __attribute__((visibility("default")))
#include <GL/osmesa.h>

class LiteWindow
{
	OSMesaContext mesaContext;
...
public:
    ...
    int IsValid ()
    {
        return mesaContext != NULL;
    }
    virtual ~LiteWindow() { if(mesaContext) OSMesaDestroyContext(mesaContext); }
    void Create(int _x = 0, int _y = 0, const char * display = NULL)
    {
        if(mesaContext) return;

		x = _x > 0 ? _x : 1;
		y = _y > 0 ? _y : 1;
		buffer.resize(x*y*4);
		mesaContext = OSMesaCreateContext(OSMESA_RGBA, NULL);
    }
    void MakeCurrent()
    {
		if(mesaContext) OSMesaMakeCurrent(mesaContext, &buffer[0], GL_UNSIGNED_BYTE, x, y);
    }
};
...
public:
    ...
    {
        if(xDisplay) return;
        if(display) LOG_OUT() << "Using display ["<<display<<"]\n";
