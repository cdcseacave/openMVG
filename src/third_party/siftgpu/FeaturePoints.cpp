...
void FeatureData::ReadFeatureDataA(std::ifstream &is, int npt, int locDim, int desDim, int szFeatureName)
{
	///////////////////////////////////////
    _npoint = npt;
...
void FeatureData::	saveSIFTA(const char* szFile, const int nDesDim)
{
	...
	for(int i = 0; i < n; i++)
	{
		...
		out << loc[1] << " " << loc[0] << " " << loc[3] << " " << loc[4] << std::endl;
...
int FeatureData::ReadSIFTB_LOC(const char* szFile)
{
	...
	if(IsValidFeatureName(sfh.szFeature) && IsValidVersionName(sfh.szVersion))
	{
		...
		if(sfh.npoint>0 && sfh.nLocDim >0 && sfh.nDesDim==128)
		{
            ...
            {
                printf("ERROR: ReadSIFTB_LOC allocation failed\r\n");
            }
			_close(fd);
			SetUpdated();
		}else
...
//	if(_stat(szFeatureFile, &buf)!=-1) return; //already saved
	...
	_write(fid, &loc[0], nf*2*sizeof(float));
	_close(fid);
}	
...
void FeatureData::SortSIFT()
{
	...
	std::vector<int> orderv(nf);
	std::vector<LTYPE> sizes(nf);
	int* order = &orderv[0];
	LTYPE* size = &sizes[0];
