...
// uncomment to enable extra checks for OpenGL calls
#ifndef _RELEASE
#define _GL_DEBUG
#endif
#ifndef GL_ASSERT
#ifdef _GL_DEBUG
#define GL_ASSERT(GLCALL) { GLCALL; const GLenum glErr(glGetError()); if (glErr != GL_NO_ERROR) { LOG_ERR() << "GL error '" << (const char*)gluErrorString(glErr) << "' in " << __FILE__ << ":" << __LINE__ << ":" << #GLCALL << std::endl; assert(glErr == GL_NO_ERROR); } }
#else
#define GL_ASSERT(GLCALL) GLCALL;
#endif
#endif
...
#else
public:
	inline static double CLOCK()				{ return 0;			}
	inline static void StopTimer()				{ 					}
	inline static void StartTimer(const char *)	{ 					}
	inline static float GetElapsedTime()		{ return 0;			}
#endif
