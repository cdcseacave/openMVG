...
{
	if(s_bag) delete s_bag, s_bag = NULL; 
}
...
void ShaderMan::UseShaderKeypoint(int texU, int texD, float dog_threshold, float edge_threshold)
{
	s_bag->s_keypoint->UseProgram();
	s_bag->SetDogTexParam(texU, texD);
	s_bag->SetDogThreshold(dog_threshold, edge_threshold);
}
...
void ShaderMan::UseShaderOrientation(int gtex, int width, int height, float sigma, int auxtex, float step, int keypoint_list)
{
	...
	//changes in v345. 
	//set sigma to 0 to identify keypoint list mode
