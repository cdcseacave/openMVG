...
protected:
	...
	std::vector<int>	_keypoint_index;
	//display vbo
...
public:
    ...
    int         _siftgpu_failed;
public:
	std::vector<float>	_keypoint_buffer;
	std::vector<float>	_descriptor_buffer;
private:
...
public:
	...
	virtual void CopyFeatureVector(float *keys, unsigned char *descriptors);
